...
class SidecarVoiceProvider:
    ...
    def transcribe(self, path_or_bytes: str | Path | bytes, content_type: str | None = None) -> TranscriptionProviderOutput:
        del content_type
        try:
            with normalize_audio(path_or_bytes, ffmpeg_bin=os.getenv("VOICE_FFMPEG_BIN", "ffmpeg"), temp_root=self.temp_root) as normalized:
        ...
        try:
            ...
            if not all(isinstance(value, str) for value in (text, language, model, provider)):
                raise ValueError
            return TranscriptionProviderOutput(text=text, language=language, model=model, provider=provider)
