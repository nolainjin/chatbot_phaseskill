...
PY
  ...
  session_token="$(jq -r '.session_token // empty' "$chat_body")"
  [ -n "$session_token" ] || fail "local cycle $i did not return a session token"
...
(
  cd "$WORK_DIR"
  node "$REPO_ROOT/scripts/voice-ui-test.mjs" --scenario hostile-events >/dev/null
)
