...
origin §6: 동일 IP는 1시간에 5회 이상 사용 불가 + API 비용 공격 방지. '5회'의 단위는 사용자 확정(2026-07-11, D05)으로 **신규 대화 세션 5회**다. capabilities.md 적대적 플래그(CAP13)는 "코드만 있고 실제 6번째가 차단되지 않는" fake-satisfy를 경고한다. 스펙 리뷰에서 세 가지 견고성 결함이 지적되어 반영됐다: XFF 신뢰 홉수 env화(O3), 카운터 파일 원자적 쓰기+락(O4), hops=0 오구성 경고(O7).
