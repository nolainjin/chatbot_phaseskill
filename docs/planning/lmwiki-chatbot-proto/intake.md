...
- "LM Wiki" = 프론트매터를 가진 마크다운 기반 지식베이스(LLM Wiki 형식)로 해석. 실제 제공받은 데이터 파일의 위치·형식은 미확인
- "외부 AI 모델은 API 키를 입력해 호출" = 운영자가 서버 측에 API 키를 설정하는 방식 (D02 답변으로 확정: Anthropic + 운영자 키 서버측 env)
- "1시간에 5회" 제한의 단위는 '대화 세션' 기준으로 해석 (메시지 단위인지 미확정 — open question)
- "10턴 미만" = 사용자 발화 기준 최대 9~10턴 캡. 경계값(10 포함 여부)은 구현 시 확정
...
- LM Wiki 지식 데이터의 실제 파일 위치와 샘플 형식은? (프론트매터 스키마 확인 필요)
- 첫 챗봇의 대상 도메인(지식셋 주제)은 무엇인가?
- rate limit "5회"의 단위: 대화 세션 5회인가, 메시지 5건인가?
- "10턴 미만"의 턴 정의: 사용자 발화 기준인가, 사용자+봇 왕복 기준인가?
- 대화 내역 저장에 대해 이용자에게 고지가 필요한 배포 대상인가? (공개 서비스 vs 내부 시연)
